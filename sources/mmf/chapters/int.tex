...
\begin{dfn}[Flux]
	...
	\end{equation}
\end{dfn}
\subsection{Integral Theorems in $\R^2$ and $\R^3$}
...
\end{thm}
\begin{proof}
	$1\implies{}2$. As with the hypothesis we have that $\exists F:D\fto\Cf\st F'(z)=f(z)\ \forall z\in D$. Given two points $z_1,z_2\in D$ and taken a smooth curve $\gamma:[a,b]\fto D\st\gamma(a)=z_1\ \wedge\ \gamma(b)=z_2$. Therefore
	\begin{equation*}
	...
	\end{equation*}
	$2\implies{}1$ Taken a point $z_0\in D$, any point $z\in D$ can be connected with a polygonal to $z_0$ since $D$ is connected. The integral of $f$ over this polygonal is obviously path-independent, hence we can define the following function
	\begin{equation*}
